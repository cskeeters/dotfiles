...
function! OpenFile()
    ...
    for path in l:flist
        call system('open '.escape(fnameescape(path), '(),'))
    endfor
endfunction
...
augroup MyNetrw | au!
    ...
    autocmd FileType netrw nmap <silent> <buffer> l <enter>
    " TODO: Need a faster way to mark files
    " autocmd FileType netrw nmap <buffer>m mfj
    autocmd FileType netrw noremap <silent> <buffer> T :call Netrw_trash()<CR>
    autocmd FileType netrw vnoremap <silent> <buffer> T :call Netrw_vtrash()<CR>

    autocmd FileType netrw noremap <silent> <buffer> o :call OpenFile()<CR>
augroup end
