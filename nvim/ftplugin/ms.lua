...
-- build
vim.keymap.set('n', '<C-p>', [[<Cmd>update<cr><Cmd>!groff -Kutf8 -Tpdf -m pdfmark -ms -t % > %:r.pdf && open -a Preview.app %:r.pdf<cr>]], { buffer=true, desc='Build/Compile to PDF' })
vim.keymap.set('n', '<C-k>', [[<Cmd>update<cr><Cmd>silent !groff -Kutf8 -Tpdf -m pdfmark -ms -t % > %:r.pdf<cr>]], { buffer=true, desc='Build/Compile to PDF' })
