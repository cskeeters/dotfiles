...
vim.keymap.set('n', '<LocalLeader>b', [["_yiWi\*[BD]<Esc>ea\*[PREV]<Esc>]], { buffer=true, desc='Bold word under cursor' })
vim.keymap.set('n', '<LocalLeader>i', [["_yiWi\*[IT]<Esc>ea\*[PREV]<Esc>]], { buffer=true, desc='Italicize word under cursor' })
vim.keymap.set('n', '<LocalLeader>`', [["_yiWi\f[CR]<Esc>ea\f[]<Esc>]], { buffer=true, desc='Monospace word under cursor' })

vim.keymap.set('v', '<LocalLeader>b', [[s\*[BD]<C-r>"\*[PREV]<Esc>]], { buffer=true, desc='Bold selected text' })
vim.keymap.set('v', '<LocalLeader>i', [[s\*[IT]<C-r>"\*[PREV]<Esc>]], { buffer=true, desc='Italicize selected text' })
vim.keymap.set('v', '<LocalLeader>`', [[s\f[CB]<C-r>"\f[]<Esc>]], { buffer=true, desc='Monospace selected text' })

-- build
vim.keymap.set('n', '<C-k>', [[<Cmd>update<cr><Cmd>silent !pdfmom -Kutf8 -t % > %:r.pdf && open -a Preview.app %:r.pdf<cr>]], { buffer=true, desc='Build/Compile to PDF' })
